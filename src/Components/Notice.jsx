...
import React, { useState, useEffect } from "react";
import { Typography, Grid, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, IconButton, Slide, Drawer, Button } from "@mui/material";
import Carousel from "react-material-ui-carousel";
import { toast, ToastContainer } from "react-toastify";
import { Edit as EditIcon, Delete as DeleteIcon, Close } from "@mui/icons-material";
import { fetchNoticeData, updateNoticeData, deleteNoticeData } from "../SubPackages/FetchNoticeData";
import NoticeUpdateForm from "../SubPackages/NoticeUpdateForm";
import NoticeCard from "../SubPackages/NoticeCard";
import NoticeData from "../SubPackages/NoticeData";
import "./Notice.css"; // Import the external CSS file
...
//   }
  ...
  return (
    <div className="Notices">
      ...
      <Carousel animation="slide" className="CarouselContainer">
        {notices.map((notice, index) => (
          <Grid key={notice.id} container spacing={2} direction={"row"}>
            (
            <NoticeCard
              title={<Typography variant="h6" className="NoticeCardTitle">{notice.title}</Typography>}
              description={<p className="NoticeCardDescription">{notice.description.slice(0, 120)+"..."}</p>}
              image={notice.image}
              link={notice.link}
              isTable={true}
              onReadMore={() => handleDrawerOpen(notice.description)}
      ...
      <TableContainer component={Paper} className="NoticesTable">
        <Table>
          <TableHead>
            <TableRow className="NoticesTableHeaderRow">
              <TableCell className="NoticesTableHeaderCell">Title</TableCell>
              <TableCell className="NoticesTableHeaderCell">Description</TableCell>
              {userType === "admin" && (
              <TableCell className="NoticesTableHeaderCell">Action</TableCell>)}
            </TableRow>
          </TableHead>
          <TableBody>
            {notices?.map((notice) => (
              <Slide direction="up" in={true} key={notice.id}>
                <TableRow className="NoticesTableRow" hover>
                  <TableCell>{notice.title}</TableCell>
                  <TableCell>
                    {notice.description.length > 100 ? (
                      <>
                        {`${notice.description.substring(0, 100)}... `}
                        <Button className="NoticesTableCellReadMoreButton" onClick={() => handleDrawerOpen(notice.description)}>Read More</Button>
                      </>
                    ) : (
                      notice.description
                    )}
